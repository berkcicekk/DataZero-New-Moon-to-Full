/**
 * Deploy the DataZero contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
import { deriveIdentitySecret, emptyProfile } from './identity';
import { createDataZeroPrivateState, witnesses } from './witnesses';

// @ts-expect-error Required for wallet sync
globalThis.WebSocket = WebSocket;

// Identifier under which this contract's private state is stored. DataZero's
// private state holds the identity secret and the local profile - both stay
// on this machine; see src/witnesses.ts.
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy DataZero to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // initialPrivateState. DataZero's constructor calls localSecretKey(), so
      // the identity secret has to be in place before the deploy transaction is
      // built. args is the constructor's arguments: DataZero takes none.
  ...
  console.log('  └────────────────────────────────────────────────────────────\n');
  console.log('  Paste this address into the Contract Address table in README.md.\n');
  ...
  console.log('  Next: npm run cli — open a campaign, then attest.\n');
}
